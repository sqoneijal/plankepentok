...
import { tableCellClass, type PaguParentRow, type PaguSubUnitRow, type PaguUPTRow } from "./init";

export default function RowPaguUPT({
   isLoading,
   content,
   paguSubUnit,
}: {
   isLoading: boolean;
   content: Array<PaguUPTRow>;
         ...
         <React.Fragment key={`upt-${row.id}`}>
            <TableRow>
               ...
               <TableCell className={cn(tableCellClass, "text-end")}>
                  <ActionButton row={row} selectedRow={selectedRow} setSelectedRow={setSelectedRow} endpoint="upt" />
               </TableCell>
            </TableRow>
            {row?.upt_master?.sub_unit?.map((item) => {
