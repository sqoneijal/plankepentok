...
import { useUpdatePaguLembaga, type FormData, type LembagaData } from "../init";

const tableCellClass = "font-medium p-1 pl-2 px-3 py-1 text-sm text-gray-900 h-8 whitespace-normal break-words";

export default function Lembaga({ id_biro, content }: { id_biro?: string; content: Array<LembagaData> }) {
   const [formData, setFormData] = useState<FormData>({});

   const { onSubmit, isPending } = useUpdatePaguLembaga({ id: formData?.id, total_pagu: Number(formData?.total_pagu), setFormData });

   const actionButton = (row: LembagaData) => {
