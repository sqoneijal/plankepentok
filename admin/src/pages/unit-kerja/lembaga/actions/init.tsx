...
export function useInitPage() {
   ...
   useEffect(() => {
      setButton(<LinkButton label="Batal" url="/unit-kerja/lembaga" />);
      return () => {
         setButton(<div />);
      };
   }, [setButton]);
...
export function useCreateData(formData: FormData, setErrors: (errors: FormData) => void) {
   const { mutate, isPending } = usePostMutation<FormData, unknown>("/unit-kerja/lembaga", (data) => ({ ...data }), [[`/unit-kerja/lembaga`]]);
...
export function useUpdateData(id: string | undefined, formData: FormData, setErrors: (errors: FormData) => void) {
   ...
   const { mutate, isPending } = usePutMutation<FormData, unknown>(`/unit-kerja/lembaga/${id}`, (data) => ({ id, ...data }), [
