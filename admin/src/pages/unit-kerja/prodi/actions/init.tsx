...
export function useOptions() {
   const { data, isLoading: isLoadingFakultas } = useApiQuery({
      url: "/options/fakultas",
   });

   const daftarFakultas = data?.results ?? [];

   return { daftarFakultas, isLoadingFakultas };
}
...
export function useInitPage() {
   ...
   useEffect(() => {
      setButton(<LinkButton label="Batal" url="/unit-kerja/program-studi" />);
      return () => {
         setButton(<div />);
      };
   }, [setButton]);
...
export function useCreateData(formData: FormData, setErrors: (errors: FormData) => void) {
   const { mutate, isPending } = usePostMutation<FormData, unknown>("/unit-kerja/prodi", (data) => ({ ...data }), [[`/unit-kerja/prodi`]]);
...
export function useUpdateData(id: string | undefined, formData: FormData, setErrors: (errors: FormData) => void) {
   ...
   const { mutate, isPending } = usePutMutation<FormData, unknown>(`/unit-kerja/prodi/${id}`, (data) => ({ id, ...data }), [
      [`/unit-kerja/prodi/${id}`],
      [`/unit-kerja/prodi`],
   ]);
