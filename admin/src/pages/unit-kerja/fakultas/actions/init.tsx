...
export function useInitPage() {
   ...
   useEffect(() => {
      setButton(<LinkButton label="Batal" url="/unit-kerja/fakultas" />);
      return () => {
         setButton(<div />);
      };
   }, [setButton]);
...
export function useCreateData(formData: FormData, setErrors: (errors: FormData) => void) {
   const { mutate, isPending } = usePostMutation<FormData, unknown>("/unit-kerja/fakultas", (data) => ({ ...data }), [[`/unit-kerja/fakultas`]]);
...
export function useUpdateData(id: string | undefined, formData: FormData, setErrors: (errors: FormData) => void) {
   ...
   const { mutate, isPending } = usePutMutation<FormData, unknown>(`/unit-kerja/fakultas/${id}`, (data) => ({ id, ...data }), [
