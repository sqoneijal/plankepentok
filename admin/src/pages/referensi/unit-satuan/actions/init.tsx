...
export function useInitPage() {
   ...
   useEffect(() => {
      setButton(<LinkButton label="Batal" url="/referensi/unit-satuan" />);
      return () => {
         setButton(<div />);
      };
   }, [setButton]);
...
export function useCreateData(formData: FormData, setErrors: (errors: FormData) => void) {
   const { mutate, isPending } = usePostMutation<FormData, unknown>("/referensi/unit-satuan", (data) => ({ ...data }), [[`/referensi/unit-satuan`]]);
...
export function useUpdateData(id: string | undefined, formData: FormData, setErrors: (errors: FormData) => void) {
   ...
   const { mutate, isPending } = usePutMutation<FormData, unknown>(`/referensi/unit-satuan/${id}`, (data) => ({ id, ...data }), [
      [`/referensi/unit-satuan/${id}`],
      [`/referensi/unit-satuan`],
   ]);
