...
class ProgramStudi extends Model
{
   ...
   {
      try {
         $data = cleanDataSubmit(['nama', 'id_fakultas', 'user_modified'], $post);
   ...
   {
      $table = $this->db->table('tb_prodi_master');
      $table->select('id, id_fakultas, nama');
      $table->where('id', $id);
   ...
   public function getData(array $params): array
   {
      ...
      $table->select('tpm.id, tpm.nama, tfm.nama as nama_fakultas, tbm.nama as nama_biro');
      $table->join('tb_fakultas_master tfm', 'tfm.id = tpm.id_fakultas', 'left');
      $table->join('tb_biro_master tbm', 'tbm.id = tfm.id_biro', 'left');
