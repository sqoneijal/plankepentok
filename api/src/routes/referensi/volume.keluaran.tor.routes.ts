...
export default async function volumeKeluaranTorRoutes(fastify: FastifyInstance, _opts: FastifyPluginOptions) {
   fastify.get(
      "/volume-keluaran-tor",
      {
      ...
      "/volume-keluaran-tor",
      {
         ...
         schema: {
            body: {
               ...
               errorMessage: {
                  required: {
                     nama: "Nama volume keluaran wajib diisi",
                  },
               },
            },
         },
   ...
   fastify.get<{
      ...
      async (request, reply) => {
         ...
         const results = await prisma.tb_mst_volume_keluaran_tor.findUnique({
            where: { id: Number(id) },
         });
   ...
   fastify.put<{
      ...
      async (request, reply) => {
         ...
         const oldData = await prisma.tb_mst_volume_keluaran_tor.findUnique({
            where: { id: Number(id) },
         });
