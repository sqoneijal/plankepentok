...
const validation = z.object({
   nama: z.preprocess((val) => (val == null ? "" : String(val)), z.string().min(1, "Nama penerima wajib diisi")),
});
...
const udpateData = async (req, res) => {
   try {
      ...
      const oldData = await db.read.tb_mst_penerima_manfaat_tor.findUnique({
      ...
      const newData = await db.write.tb_mst_penerima_manfaat_tor.update({
         where: { id: Number.parseInt(id) },
         data: {
            nama,
            keterangan,
      ...
      await logAudit(user_modified, "UPDATE", "tb_mst_penerima_manfaat_tor", req.ip, { ...oldData }, { ...newData });
